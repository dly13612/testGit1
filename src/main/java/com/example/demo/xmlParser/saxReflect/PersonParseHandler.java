...
public class PersonParseHandler extends DefaultHandler {
    // xml节点名称
    ...
    // 构造方法 做些初始化工作
    public PersonParseHandler(String nodeName, Class clazz) {
                ...
                for (int j = 0; j < declaredFields.length; j++) {
                    if ("childNode".equals(declaredFields[j].getName())) {
                        ...
                        declaredFields[j].set(object,
                                PersonSaxFactory.xmlToObject(url, nodeName, clazz));
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
